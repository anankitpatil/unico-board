...
export default function Home() {
  ...
  const search = (collection) => {
    if (searchQuery.length > 1) {
      let newCollection = {};
      for (let key in cardCollection) {
        const set = cardCollection[key].filter((card) => {
          console.log(card.title.indexOf(searchQuery));
          return (
            card.title.toLowerCase().indexOf(searchQuery) !== -1 ||
            card.label.toLowerCase().indexOf(searchQuery) !== -1
          );
        });
        newCollection[key] = set;
      }
      ...
      <DragDropContext onDragEnd={handleOnDragEnd}>
        <main className={styles.main}>
          <div className={styles.scroller} ref={scrollerRef}>
            {Object.keys(search(cardCollection)).length > 0 &&
              Object.entries(search(cardCollection)).map(
                ([type, cardSet], i) => {
                  return (
                    <DragDropList cardSet={cardSet} key={i} title={type} />
                  );
                }
              )}
          </div>
